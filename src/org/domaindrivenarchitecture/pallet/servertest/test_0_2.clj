...
(ns org.domaindrivenarchitecture.pallet.servertest.test-0-2
  (:require
...
(defn server-test
  ...
  (pallet.action/declare-delayed-crate-action 
    server-test
    (fn [resource-key test-fn & {:keys [use-transformed-out] :or {use-transformed-out true}}]
      (let [writer (new java.io.StringWriter)
            output-nv @(-> (crate/get-settings :dda-servertest-resources) resource-key)
            out (if use-transformed-out (:transformed-out output-nv ) (:out output-nv))
